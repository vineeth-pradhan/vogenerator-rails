...
def write_into_vo_file(new_vo_file,klass)
  new_vo_file.puts(package_name)
  new_vo_file.puts('{')
    new_vo_file.printf(insert_space(5)+"import com.ndsi.core.valueobjects.base.NdsVersionedVO;; \n")
    new_vo_file.printf(insert_space(5)+"[RemoteClass(alias='#{klass.name}')] \n")
    new_vo_file.printf(insert_space(5)+"[Bindable]")
    new_vo_file.print("\n")
    
    
    new_vo_file.printf(insert_space(5)+"public dynamic class #{klass.name} extends NdsVersionedVO \n")
    ...
    new_vo_file.print("\n")
    new_vo_file.print(insert_space(9))
    new_vo_file.printf("public function #{klass.name}() \n")
    new_vo_file.print(insert_space(9))
    new_vo_file.printf("{ \n")
    new_vo_file.print(insert_space(9))
    new_vo_file.printf("} \n")
  ...
  close(new_vo_file)  
  File.delete(new_vo_file)
end

def package_name
  "package com.cyncerp.valueobjects" 
end

def methodize(str)
  # str should be in camel case
  ch = ""
...
def build_columns_hash(table_columns)
  table_arrys = []
